...
!GET THE SECOND DERIVATIVE FOR TENSOR TAU IN RESPECT TO x
subroutine exchange_band_dtaudm()
   implicit none
   integer:: status_array(MPI_STATUS_SIZE,12), ierr
   integer,dimension(12)::req 
   integer:: tag,tag2,k,tama
      ...
      tama=3*3*(e_s-(s_s)+1)
      tag = 1
      call MPI_SendRecv ( dtaudm(1:3,d_s-2:d_s  ,s_s:e_s), tama, MPI_DOUBLE_PRECISION, right, tag,&
                          dtaudm(1:3,i_s-3:i_s-1,s_s:e_s), tama, MPI_DOUBLE_PRECISION, left , tag,&
                       cartesian_comm, req(1), ierr)
      tag = 2
      call MPI_SendRecv ( dtaudm(1:3,i_s:i_s+2  ,s_s:e_s), tama, MPI_DOUBLE_PRECISION, left , tag,&
                          dtaudm(1:3,d_s+1:d_s+3,s_s:e_s), tama, MPI_DOUBLE_PRECISION, right, tag,&
                       cartesian_comm, req(1), ierr)
  return
end subroutine

!GET THE SECOND DERIVATIVE FOR TENSOR TAU IN RESPECT TO Y
!como te falei no outro comentario, 
!vc nao precisa fazer a derivada segunda do vetor tau, e so uma deriva 
!primeira com as metricas nas derivadas que compoem o tau. Depois e multiplicar 
!pelas metrica, com se fosse uma derivada primeira.
subroutine exchange_band_dtaudn()
   implicit none
   integer:: status_array(MPI_STATUS_SIZE,12), ierr
   integer,dimension(12)::req 
   integer:: tag,tag2,k,tama
    ...
    tama=3*3*(d_s-(i_s)+1)
      tag = 1
      call MPI_SendRecv ( dtaudn(1:3,i_s:d_s,e_s-2:e_s   ), tama, MPI_DOUBLE_PRECISION, top, tag,&
                          dtaudn(1:3,i_s:d_s,s_s-3:s_s-1 ), tama, MPI_DOUBLE_PRECISION, bot, tag,&
                          cartesian_comm, req(1), ierr)
      tag = 2
      call MPI_SendRecv ( dtaudn(1:3,i_s:d_s,s_s:s_s+2)  , tama, MPI_DOUBLE_PRECISION, bot, tag,&
                          dtaudn(1:3,i_s:d_s,e_s+1:e_s+3), tama, MPI_DOUBLE_PRECISION, top, tag,&
                          cartesian_comm, req(1), ierr)
      return
end subroutine
...
!  ! Bottom to Top
      ...
      call MPI_SendRecv ( temp(i_s:d_s,e_s-2:e_s   ), tama, MPI_DOUBLE_PRECISION, top, tag,&
                          temp(i_s:d_s,s_s-3:s_s-1 ), tama, MPI_DOUBLE_PRECISION, bot, tag,&
                          cartesian_comm, req(1), ierr)
!**********************************************************
!  !  Top to Bottom 

      tag = 2
      call MPI_SendRecv ( temp(i_s:d_s,s_s:s_s+2)  , tama, MPI_DOUBLE_PRECISION, bot, tag,&
                          temp(i_s:d_s,e_s+1:e_s+3), tama, MPI_DOUBLE_PRECISION, top, tag,&
                          cartesian_comm, req(1), ierr)
...
!**********************************************************
      ...
      call MPI_SendRecv ( DTEMPDN(d_s-2:d_s  ,s_s:e_s), tama, MPI_DOUBLE_PRECISION, right, tag,&
                          DTEMPDN(i_s-3:i_s-1,s_s:e_s), tama, MPI_DOUBLE_PRECISION, left , tag,&
                       cartesian_comm, req(1), ierr)

!**********************************************************
!right to left 
      tag = 2
      call MPI_SendRecv ( DTEMPDN(i_s:i_s+2  ,s_s:e_s), tama, MPI_DOUBLE_PRECISION, left , tag,&
                          DTEMPDN(d_s+1:d_s+3,s_s:e_s), tama, MPI_DOUBLE_PRECISION, right, tag,&
                       cartesian_comm, req(1), ierr)
...
!  ! Bottom to Top
      ...
      call MPI_SendRecv ( DTEMPDN(i_s:d_s,e_s-2:e_s   ), tama, MPI_DOUBLE_PRECISION, top, tag,&
                          DTEMPDN(i_s:d_s,s_s-3:s_s-1 ), tama, MPI_DOUBLE_PRECISION, bot, tag,&
                          cartesian_comm, req(1), ierr)
!**********************************************************
!  !  Top to Bottom 

      tag = 2
      call MPI_SendRecv ( DTEMPDN(i_s:d_s,s_s:s_s+2)  , tama, MPI_DOUBLE_PRECISION, bot, tag,&
                          DTEMPDN(i_s:d_s,e_s+1:e_s+3), tama, MPI_DOUBLE_PRECISION, top, tag,&
                          cartesian_comm, req(1), ierr)
...
!**********************************************************
      ...
      call MPI_SendRecv ( dtempdm(d_s-2:d_s  ,s_s:e_s), tama, MPI_DOUBLE_PRECISION, right, tag,&
                          dtempdm(i_s-3:i_s-1,s_s:e_s), tama, MPI_DOUBLE_PRECISION, left , tag,&
                       cartesian_comm, req(1), ierr)

!**********************************************************
!right to left 
      tag = 2
      call MPI_SendRecv ( dtempdm(i_s:i_s+2  ,s_s:e_s), tama, MPI_DOUBLE_PRECISION, left , tag,&
                          dtempdm(d_s+1:d_s+3,s_s:e_s), tama, MPI_DOUBLE_PRECISION, right, tag,&
                       cartesian_comm, req(1), ierr)
...
!**********************************************************
      ...
      call MPI_SendRecv ( k1(d_s-2:d_s  ,s_s:e_s), tama, MPI_DOUBLE_PRECISION, right, tag,&
                          k1(i_s-3:i_s-1,s_s:e_s), tama, MPI_DOUBLE_PRECISION, left , tag,&
                       cartesian_comm, req(1), ierr)

!**********************************************************
!right to left 
      tag = 2
      call MPI_SendRecv ( k1(i_s:i_s+2  ,s_s:e_s), tama, MPI_DOUBLE_PRECISION, left , tag,&
                          k1(d_s+1:d_s+3,s_s:e_s), tama, MPI_DOUBLE_PRECISION, right, tag,&
                       cartesian_comm, req(1), ierr)
...
!  ! Bottom to Top
      ...
      call MPI_SendRecv ( k1(i_s:d_s,e_s-2:e_s   ), tama, MPI_DOUBLE_PRECISION, top, tag,&
                          k1(i_s:d_s,s_s-3:s_s-1 ), tama, MPI_DOUBLE_PRECISION, bot, tag,&
                          cartesian_comm, req(1), ierr)
!**********************************************************
!  !  Top to Bottom 

      tag = 2
      call MPI_SendRecv ( k1(i_s:d_s,s_s:s_s+2)  , tama, MPI_DOUBLE_PRECISION, bot, tag,&
                          k1(i_s:d_s,e_s+1:e_s+3), tama, MPI_DOUBLE_PRECISION, top, tag,&
                          cartesian_comm, req(1), ierr)
